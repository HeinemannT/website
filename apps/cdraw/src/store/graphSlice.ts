...
export const createGraphSlice: StateCreator<StoreState, [["zustand/immer", never]], [], GraphSlice> = (set, get) => ({
    ...
    deleteNode: (id) => set((state) => {
        ...
        if (nodeIndex !== -1) {
            // Optimization: Create a Quick Lookup Map for Nodes to avoid O(N) search inside loop
            // This reduces complexity from O(E*N) to O(N + E)
            const nodeMap = new Map();
            state.nodes.forEach(n => nodeMap.set(n.id, n));

            // 1. Filter Edges & Cleanup Linked Properties (One Pass)
            state.edges = state.edges.filter(edge => {
                const isConnected = edge.source === id || edge.target === id;
                if (isConnected) {
                    // If we are deleting the target of a smart edge, we must cleanup the property on the source
                    if (edge.source !== id && edge.data?.linkedPropertyId) {
                        const sourceNode = nodeMap.get(edge.source);
                        if (sourceNode) {
                            sourceNode.data.linkedProperties = sourceNode.data.linkedProperties.filter((p: string) => p !== edge.data?.linkedPropertyId);
                        }
                    }
            ...
            // 2. Handle Children (Unparent them to prevent loss)
