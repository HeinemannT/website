...
export default function Lesson2c() {
  return (
    <LessonShell lesson={lesson} retrieval={retrieval}>
      ...
      <MathBlock>
        <p>“Coverage” is a search-saturation signal, not a completeness measure. Across techniques or rounds <span className="eq">1…k</span>, let <span className="eq">n_k</span> be the count of <em>new</em> distinct risks the k-th adds. Diminishing returns is:</p>
        <p style={{ textAlign: 'center' }}><span className="eq">n_1 ≥ n_2 ≥ … ≥ n_k → 0</span></p>
      ...
      <p className="measure">A worked correction, because this is the most common mistake. A workshop participant offers “we have cyber risk.” It names a domain, so it can be neither owned nor sized nor controlled — there is no specific event to attach a probability to. Apply the discipline: pick the actual scenario, then split it. <em>Cause:</em> a phishing email gets an employee to surrender credentials. <em>Event:</em> ransomware encrypts the order-management ERP. <em>Consequence:</em> order capture and dispatch are down for several days, missing retailer service levels and incurring recovery cost (Meridian sizes this as R2, ≈€6m). Now there is a cause to put a preventive control on, a consequence to size in 2d and finance in 2f, and an owner who can plausibly be the CISO. That move — from one-word topic to a three-part, ownable, assessable line — is the entire craft of this lesson.</p>
