...
export default function Lesson46() {
  return (
    <LessonShell lesson={lesson} retrieval={retrieval}>
      ...
      <p className="measure"><strong>Meridian, worked.</strong> Treasury runs a €60m reserve portfolio: roughly 60% in bonds (volatility about 6%) and 40% in equities (volatility about 18%), with a calm-time correlation around 0.3. The “just add them up” instinct points at the weighted-average volatility: 0.6 × 6% + 0.4 × 18% = 10.8%. That weighted average is what you’d suffer if the two moved in perfect lockstep — the no-benefit case. But at the actual correlation of 0.3 the offset kicks in and the portfolio volatility works out to about 9.0% (you’ll build the exact calculation in Stage 3). The difference — nearly 2 percentage points of volatility on €60m, on the order of €1m of risk that simply isn’t there — is the diversification benefit, claimed for free.</p>

      <p className="measure">Now imagine a 2008-style crisis. Push the correlation toward 1 and the portfolio volatility climbs back toward 10.8%: the benefit evaporates and the reserve that looked comfortably steady starts moving like a single concentrated bet — exactly when Meridian, facing a demand downturn that is already squeezing EBITDA toward its covenant, can least afford a hit to its reserves. The free lunch was real in calm weather and gone in the storm.</p>
