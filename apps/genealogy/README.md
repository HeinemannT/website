...
The map includes a prominent chronological year slider with event ticks, a readable selected date, reset and previous/next dated-record controls. **Play** steps actual records every 2.5 seconds, including separate records in the same year; it does not rush through empty years. Dragging the slider, interacting with the map/list, opening a source, hiding the tab or closing the explorer pauses playback. Reduced motion disables map flights. Undated/unresolved records remain accessible through the timeline link and list, without receiving a year. Undated place markers remain visible at every cursor year; dated markers appear when their year is reached.

The reviewed chronology contains 191 records: the original fourteen place records plus 177 newly indexed manuscript records. Of these, 175 have a year or year range; sixteen have no resolved year. There is no event-kind dropdown. Most added entries are life dates without recorded coordinates; only three additional burial/reburial records have event-specific approximate locations. Selecting an unlocated life event stops any camera flight and displays **Location unrecorded**. The compact list keeps reviewed uncertainty labels, immediate manuscript links and an expandable transcription/translation. **Previous place / Next place** skips unlocated records and records at the selected coordinates; it follows dated records, then undated places without assigning them dates. The separate timeline controls still step every dated record. **Places reached** fits the currently reached mapped records. **Guangdong** and **South Africa** frame their geographic region, while the timeline still determines which dated markers are visible.
