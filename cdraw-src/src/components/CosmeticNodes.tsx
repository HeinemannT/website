...
export const GroupNode = memo(({ id, data, selected }: NodeProps<NodeData>) => {
    ...
    return (
        <div className={clsx(
            "relative group/box transition-all h-full w-full min-w-[50px] min-h-[50px]", // Enforce min dimensions
            ...
            {viewMode === 'architect' ? (
                ...
                // Simple Mode: Just a floating label at the top, or cleaner look
                <div
                    className="absolute -top-6 left-0 w-full pointer-events-auto flex"
                    style={{ justifyContent: textAlign === 'center' ? 'center' : (textAlign === 'right' ? 'flex-end' : 'flex-start') }}
                >
                    <input
                        className="bg-transparent border-none text-xs font-bold text-gray-500 uppercase tracking-wide px-1 py-0.5"
                        value={data.label}
                        onChange={(e) => updateNodeMetadata(id, { label: e.target.value })}
                        placeholder="GROUP"
                    />
